...
TRANSLATIONS = {
    # ---------------------------------------------------------------- App meta
    'app_title':            {'tr': '🪨 Gözenek ve Renk Tespit Aracı',
                             'en': '🪨 Pore Segmentation Suite'},
    'app_caption':          {'tr': 'Doğaltaşlar için interaktif segmentasyon — DoG / MSER / Sauvola / Color-Distance + per-stone renk paleti',
    ...
    'builtin_sample_label': {'tr': '📷 Gömülü örnek numune', 'en': '📷 Built-in sample'},
    'builtin_none': {'tr': 'seç', 'en': 'select'},
    'ag_load_samples': {'tr': '🧪 Örnek before/after çiftlerini yükle', 'en': '🧪 Load sample before/after pairs'},
    'ag_samples_active': {'tr': '4 gömülü traverten çifti yüklendi (GT · KT · NT · PT) — Analizi çalıştırabilirsiniz', 'en': '4 bundled travertine pairs loaded (GT · KT · NT · PT) — you can run the analysis'},
    ...
    'cb_footer_default': {'tr': 'Generated by Gözenek ve Renk Tespit Yazılımı v1.3 (Sert 2026)', 'en': 'Generated by Pore Segmentation Suite v1.3 (Sert 2026)'},
    ...
    'cite_title': {'tr': '📑  **Atıf yap!**', 'en': '📑  **Cite!**'},
    'changelog_title': {'tr': '🆕  **Versiyon Güncellemeleri**', 'en': '🆕  **Version Updates**'},
...
COLOR_SCIENCE_TR_TO_EN = {
    # v1.3.0 metric-specific classes
    "Algılanamaz (below perceptibility threshold)": "Below perceptibility threshold (dE00 < PT)",
    "Algılanabilir, kabul edilebilir (perceptible, acceptable)": "Perceptible but acceptable (PT <= dE00 < AT)",
    "Kabul edilebilirlik eşiğinin üzerinde (beyond acceptability threshold)": "Beyond acceptability threshold (dE00 >= AT)",
    "Fark edilmez (not noticed)": "Not noticed",
    "Sadece deneyimli gözlemci fark eder (experienced observer only)": "Experienced observer only",
    "Deneyimsiz gözlemci de fark eder (unexperienced observer notices)": "Unexperienced observer also notices",
    "Açık fark (clear difference)": "Clear difference",
    "İki farklı renk (two different colours)": "Two different colours",
    "Doğrulanmış eşik yok (no validated threshold for CIE94)": "No validated threshold for CIE94",
...
TRANSLATIONS.update({
 ...
 'psd_xlabel': {'tr': 'Eşdeğer gözenek çapı (µm)', 'en': 'Equivalent pore diameter (µm)'},
 'psd_res_label': {'tr': 'Görüntü çözünürlüğü altında', 'en': 'below imaging resolution'},
 'p2_fam_classical': {'tr': 'Klasik', 'en': 'Classical'},
 'p2_fam_blob': {'tr': 'Leke/bölge', 'en': 'Blob/region'},
 'p2_fam_clustering': {'tr': 'Kümeleme', 'en': 'Clustering'},
 ...
 'psd_note': {'tr': 'Not: Bu, görüntüden türetilen 2B, yüzey, alan-ağırlıklı bir gözenek boyut dağılımıdır. Cıva porozimetresine (MIP) bir benzeridir, onun yerini tutmaz. MIP 3B gözenek dağılımlarını hacimce ölçer. Bu yöntem ise alan olarak ölçer.',
              'en': 'Note: this is an image-derived 2D, surface, area-weighted pore-size distribution. It is an analogue to, not a substitute for, mercury intrusion porosimetry (MIP). MIP measures 3D pore distributions by volume, whereas this method measures by area.'},
...
TRANSLATIONS.update({
 ...
 'demo_pick_label': {'tr': 'Örnek numune seç', 'en': 'Choose a sample'},
 'demo_caption': {'tr': 'Gerçek traverten yüzeyleri (KT/NT/PT/GT, öncesi/sonrası) — yüklemeden deneyin.',
                  'en': 'Real travertine surfaces (KT/NT/PT/GT, before/after) — try without uploading.'},
...
# ---- v1.3.0: fixed-percentile detector warning ----
TRANSLATIONS.update({
 'fixed_fraction_warning': {
  'tr': '⚠️ Bu yöntem filtre yanıtını sabit bir yüzdelikte eşikler; taştan bağımsız olarak yaklaşık sabit bir alan payı işaretler. Gözenek konumu ve morfolojisi için uygundur, kalibrasyon olmadan porozite ölçümü için kullanılmamalıdır.',
  'en': '⚠️ This method thresholds its filter response at a fixed percentile and marks a nearly constant area fraction regardless of the stone. Use it for pore location and morphology; do not use it to quantify porosity without calibration.'},
})
